...
export class HomePage implements OnInit, OnDestroy {
  ...
  async getReservas() {
    ...
    // Primero obtener los anuncios activos para filtrar las reservas
    this.api.getAds(token).subscribe(
      async (adsResponse: any) => {
        const activeAds = adsResponse.data;
        const activeAdIds = activeAds.map((ad: any) => ad.gar_id);
        
        // Ahora obtener las reservas y filtrarlas
        this.api.getReservations(token).subscribe(
          async (response: any) => {
            const ahora = new Date();
            
            // Filtrar reservas que pertenecen a anuncios activos
            const filteredReservations = response.data.filter((element: any) => {
              return activeAdIds.includes(element.gar_id);
            });

            filteredReservations.forEach((element: any) => {
            ...
            this.data = filteredReservations;
